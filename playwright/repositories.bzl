...
_ATTRS = {
    "playwright_version": attr.string(mandatory = True, values = TOOL_VERSIONS.keys()),
    "platform": attr.string(mandatory = True, values = PLATFORMS.keys()),
}

def _playwright_repo_impl(repository_ctx):
    url = "https://github.com/someorg/someproject/releases/download/v{0}/playwright-{1}.zip".format(
        repository_ctx.attr.playwright_version,
        repository_ctx.attr.platform,
    )
    repository_ctx.download_and_extract(
        url = url,
        integrity = TOOL_VERSIONS[repository_ctx.attr.playwright_version][repository_ctx.attr.platform],
    )
    build_content = """# Generated by playwright/repositories.bzl
load("@com_mrmeku_rules_playwright//playwright:toolchain.bzl", "playwright_toolchain")
...
def playwright_register_toolchains(name, register = True, **kwargs):
    ...
    Args:
        ...
        **kwargs: passed to each playwright_repositories call
    """
